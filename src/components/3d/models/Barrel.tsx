...
export default function Barrel({
  ...
  const { scene } = useGLTF("/3D/new_thin_barrel.glb");
  ...
  useEffect(() => {
    if (scene) {
      ...
      console.log(`[Barrel] Loaded - Size: ${size.x.toFixed(2)} x ${size.y.toFixed(2)} x ${size.z.toFixed(2)}`);
    }
  }, [scene, scale]);
...
useGLTF.preload("/3D/new_thin_barrel.glb");
